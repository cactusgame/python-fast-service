...
@api(db="tests/resources/data.db", type="sqlite", sql="select age from USER where age > 90")
def datasource(table_name=None, data=None):
    return data


# In fact, you can write the fix part of the `DATA_SOURCE` in config.py
@api(type="sql", host=ac.get_value("test_rds_host"), port=3306, username=ac.get_value("test_rds_username"),
